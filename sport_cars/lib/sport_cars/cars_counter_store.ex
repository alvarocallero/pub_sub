defmodule SportCars.CarsCounterStore do
  ...
  def get_request_counter(request_name) do
    GenServer.call(__MODULE__, {:get_request_counter, request_name})
  end

  def increment_request_counter(request_name) do
    GenServer.cast(__MODULE__, {:increment_request_counter, request_name})
  end
  ...
  def handle_call({:get_request_counter, request_name}, _from, state) do
    {:reply, Map.get(state, request_name), state}
  end

  def handle_cast({:increment_request_counter, request_name}, state) do
    new_state = Map.put(state, request_name, Map.get(state, request_name, 0) + 1)
